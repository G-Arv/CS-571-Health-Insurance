...
function createPieChart(state) {
    let color = d3.scaleOrdinal(["rgb(115, 146, 217)", "#f4a261"]);
    ...
    function makePie(data) {
        ...
        legend.append("rect").attr("width", 15).attr("height", 15).attr("fill", (d, i) => {
                return color(i)
              });
...
function createBarChart(state) {
    let color = d3.scaleOrdinal(["rgb(115, 146, 217)", "#fb9259"]);

    const data = d3.csv(`healthInsurance.csv`).then(output => {
        for(let i = 0; i < output.length; ++i) {
...
function createPieChart(state) {
    let color = d3.scaleOrdinal(["rgb(115, 146, 217)", "#f4a261"]);
    const data = d3.csv(`data/alldata.csv`).then(output => {
        console.log(output);
        let uninsured = 0;
        let total = 0;
        let count = 0;

        for (let i = 0; i < output.length; ++i) { //gets all data from the state selected
            if (output[i].State == state) { //sum of column(s)
                uninsured += parseFloat(output[i]["Uninsured"]);
                total += parseFloat(output[i]["Total"]);
                count++; // Count the rows for the state
            }
        }
        
        if (count > 0) { //avg of the years in that state
            let avguninsured =  uninsured / count;
            let avgtotal = total / count;
            makePie([avguninsured, avgtotal]); //Create Chart
        }
    });
    ...
    function makePie(data) {
        const pieSearch = ["Uninsured", "Total"];
        const pieData = data;
        let svg = d3.select("#pie-chart");
        const width = 500;
        const height = 500;
        ...
        let pie = d3.pie(); // Creates the pie
        let arc = d3.arc().innerRadius(0).outerRadius(radius); //Creates arcs
        // Generate the groups for the arcs
        let arcs = g.selectAll("arc").data(pie(pieData)).enter().append("g").attr("class", "arc");
        arcs.append("path").attr("fill", (d, i) => color(i)).attr("d", arc);
        // Creates a legend for the pie chart
        let legend = svg.selectAll(".legend").data(pie(pieData)).enter().append("g")
                        .attr("transform", (d, i) => {return "translate(" + (width / 4) + "," + (i * 15 + 400) + ")";}).attr("class", "legend");

        legend.append("rect").attr("width", 15).attr("height", 15).attr("fill", (d, i) => {return color(i);});
        legend.append("text").text(((d, i) => {return pieSearch[i].replaceAll("_", " ");})).style("font-size", 12).attr("y", 10).attr("x", 20);
    }
}


let currState = 'California';
createPieChart(currState);
